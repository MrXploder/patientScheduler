(function(){
	...
	branchesFactory.$inject = ['$localResource', "$localStorage", "$resource", "$sessionStorage", "$http"];

	function branchesFactory($localResource, $localStorage, $resource, $sessionStorage, $http){
		const _resourceName = "branches";
		...
		Resource.$pull = function(){
			return $http.get(`/${_resourceName}/${$sessionStorage.currentUser.branch_id}`).then(function(response){
		...
		Resource.$push = function(){
			return $http.post(`/${_resourceName}/${$sessionStorage.currentUser.branch_id}`, $localStorage[_resourceName])
		}

		/*this method gets all the resources from serverDB*/
		Resource.$findInServer = function(_q){
			const OriginResource = $resource(`/${_resourceName}/:id`, {id: "@_id"}, {create: {method: "POST"}, update: {method: "PUT"}});
