(function(){
	...
	operatorsFactory.$inject = ['$localResource', "$localStorage", "$resource", "$sessionStorage", "$http"];

	function operatorsFactory($localResource, $localStorage, $resource, $sessionStorage, $http){
		const _resourceName = "operators";
		...
		Resource.$findInServer = function(_q){
			const OriginResource = $resource("/operators/:id", {id: "@_id"}, {create: {method: "POST"}, update: {method: "PUT"}});
